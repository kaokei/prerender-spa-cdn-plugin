...
function PrerenderSPACdnPlugin (...args) {
  this._options = {}
  var rendererOptions = {}
  // Normal args object.
  if (args.length === 1) {
...
PrerenderSPACdnPlugin.prototype.apply = function (compiler) {
  ...
  const afterEmit = async (compilation, done) => {
    ...
    try {
      ...
      const msg = '[prerender-spa-cdn-plugin] Unable to start proxy server!'
      console.error(msg)
    ...
    // Create dirs and write prerendered files.
      ...
      .then(r => {
        ...
        this.browserProxyServer && this.browserProxyServer.close(() => { console.log(`\nproxy server http://localhost:${browserProxyPort} is close`) })
        done()
      })
      .catch(() => {
        PrerendererInstance.destroy()
        const msg = '[prerender-spa-cdn-plugin] Unable to prerender all routes!'
        console.error(msg)
        compilation.errors.push(new Error(msg))
        this.browserProxyServer && this.browserProxyServer.close(() => { console.log(`\nproxy server http://localhost:${browserProxyPort} is close`) })
        done()
